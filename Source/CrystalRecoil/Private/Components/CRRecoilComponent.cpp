...
void UCRRecoilComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	if (!RecoilToApply.IsNearlyZero())
	{
		...
		{
			// A rejected delta was not generated by this component and must not be
			// subtracted from player input on the next frame.
			DeltaRecoilRotation = FRotator::ZeroRotator;
		}
	}
	...
	if (RecoilPattern->RecoveryDelay >= 0.f && RecoilToApply.IsNearlyZero() && !RecoilToRecover.IsNearlyZero(0.001))
	{
		if (LastFireTime + RecoilPattern->RecoveryDelay < World->GetTimeSeconds())
		{
			...
			DeltaRecoveryRotation = FMath::RInterpConstantTo(FRotator::ZeroRotator, RecoilToRecover, DeltaTime, CurrentRecoverySpeed);
			...
			{
				DeltaRecoveryRotation = ApplyInputToControllerAndMeasure(Controller, DeltaRecoveryRotation);
				RecoilToRecover += DeltaRecoveryRotation;
			}
			else
			{
				// See the recoil path above: rejected recovery is not generated input.
...
void UCRRecoilComponent::ApplyShot()
{
	...
	const UWorld* World = GetWorld();
	if (!Controller || !Controller->IsLocalPlayerController() || !RecoilPattern || !World)
	{
		return;
	}
	...
	// Preserve any uplift that has not yet been applied. Recompute the movement
	// profile for the combined remainder so rapid fire cannot discard a shot.
	RecoilToApply += FRotator(-RecoilPositionDelta.Y, RecoilPositionDelta.X, 0.0);
	const float RemainingRecoilLength = FMath::Sqrt(RecoilToApply.Pitch * RecoilToApply.Pitch + RecoilToApply.Yaw * RecoilToApply.Yaw);

	// Kinematics: v0 = 2d/T, a = 2d/T^2
	// Guarantees the combined pending recoil is consumed in UpliftDuration.
	CurrentUpliftDeceleration = (2.f * RemainingRecoilLength) / (UpliftDuration * UpliftDuration);
	CurrentRecoilSpeed = 2.f * RemainingRecoilLength / UpliftDuration;
...
FRotator UCRRecoilComponent::ApplyInputToControllerAndMeasure(AController* InTargetController, const FRotator& Input)
{
	if (!InTargetController)
	{
	...
	const FRotator RotationBeforeInput = InTargetController->GetControlRotation();
	ApplyInputToController(InTargetController, Input);
	const FRotator AppliedControlRotation = (InTargetController->GetControlRotation() - RotationBeforeInput).GetNormalized();

	// ApplyInputToController subtracts pitch and adds yaw. Convert the measured
	// controller movement back into the component's input convention.
	return FRotator(-AppliedControlRotation.Pitch, AppliedControlRotation.Yaw, 0.f);
}
